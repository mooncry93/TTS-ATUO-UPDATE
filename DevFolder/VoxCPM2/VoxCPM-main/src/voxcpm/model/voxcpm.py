...
class VoxCPMModel(nn.Module):
    ...
    def _generate(
        ...
        # Prevent token count inflation on byte-tokenized Khmer texts
        resolved_max_len = min(int(target_text_length * retry_badcase_ratio_threshold + 10), max_len)
        char_len = len(target_text)
        if char_len < target_text_length:
            resolved_max_len = min(resolved_max_len, int(char_len * 3.5 + 150))
    ...
    def from_local(
        ...
        if not training:
            from .utils import pick_runtime_dtype
            resolved_dtype = pick_runtime_dtype(model.device, model.config.dtype)
            lm_dtype = get_dtype(resolved_dtype)
